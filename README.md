...
**A note on cloud/streaming access:** `model_load.ipynb` prefers `intake-esgf`'s cloud catalogs (Pangeo/ESGF Zarr stores) over direct ESGF file downloads where available, since it's normally much faster — but streaming access is less reliable than a plain download: requests can stall, time out, or fail partway through for reasons unrelated to whether the data actually exists (see the ESGF data-node flakiness note under [Known Issues](#known-issues-and-caveats)). `intake-esgf` documents these streaming-specific failure modes here: https://intake-esgf.readthedocs.io/en/latest/stream/. When a variable that should exist keeps failing via the cloud catalog, `model_load.ipynb`'s retry logic falls back to a direct ESGF/OPeNDAP download rather than treating it as a genuine archival gap.
